	...
	methods: {
		...
		getChart1Options() {
			return {
				chart: {
					...
					shadow: {
						enabled: true,
						color: 'rgba('+ appVariable.color.whiteRgb + ', .5)',
						top: 18,
						left: 7,
						blur: 10,
						opacity: 1
				...
				dataLabels: {
					...
					background: {
						...
						borderRadius: 2,
						borderWidth: 0,
						borderColor: appVariable.color.white,
						opacity: 0.9,
						dropShadow: {
							enabled: false,
							top: 1,
				...
				grid: {
					row: { colors: ['rgba('+ appVariable.color.whiteRgb + ', .25)', 'transparent'], opacity: 0.5 }
				},
				markers: { size: 4 },
		...
		getChart4Options() {
			return {
				...
				dataLabels: {
					enabled: true,
					offsetX: -6,
					style: { fontSize: '12px', colors: [appVariable.color.white] }
				},
		...
		getChart5Options() {
			return {
				...
				dataLabels: { enabled: false },
				stroke: { width: [0, 0, 3] },
				colors: [appVariable.color.theme, 'rgba('+ appVariable.color.whiteRgb + ', .5)', appVariable.color.orange],
				...
				yaxis: [
					...
					},
					{
						...
						axisTicks: { show: true, },
						axisBorder: { show: true, color: 'rgba('+ appVariable.color.whiteRgb + ', .5)' },
						labels: { style: { color: 'rgba('+ appVariable.color.whiteRgb + ', .5)' } },
						title: { text: "Operating Cashflow (thousand crores)", style: { color: 'rgba('+ appVariable.color.whiteRgb + ', .5)' } },
					}, 
					{
						seriesName: 'Revenue',
						opposite: true,
		...
		getChart7Options() {
			return {
				chart: { type: 'bubble' },
				dataLabels: { enabled: false },
				colors: [appVariable.color.theme, appVariable.color.orange, 'rgba('+ appVariable.color.whiteRgb + ', .5)', appVariable.color.pink],
				fill: { opacity: 0.8 },
		...
		getChart8Options() {
			return {
				...
				colors: [appVariable.color.theme, appVariable.color.orange, 'rgba('+ appVariable.color.whiteRgb + ', .5)'],
				xaxis: {
					tickAmount: 10,
					labels: {
		...
		getChart10Options() {
			return {
				...
				stroke: { show: false },
				colors: [ 'rgba('+ appVariable.color.pinkRgb +', .75)',  'rgba('+ appVariable.color.orangeRgb +', .75)',  'rgba('+appVariable.color.themeRgb +', .75)', 'rgba('+ appVariable.color.whiteRgb + ', .5)',  'rgba('+appVariable.color.indigoRgb +', .75)'],
			...
			return {
				...
				plotOptions: {
					radialBar: {
						...
						track: { background: 'rgba('+ appVariable.color.whiteRgb + ', .25)' },
			...
			return {
				...
				plotOptions: {
					radar: {
						size: 140,
						polygons: {
							strokeColors: 'rgba('+ appVariable.color.whiteRgb +', .25)',
							strokeWidth: 1,
							connectorColors: 'rgba('+ appVariable.color.whiteRgb +', .25)',
							fill: {
								colors: ['rgba('+appVariable.color.gray300Rgb +', .25)', 'rgba('+ appVariable.color.whiteRgb + ', .25)']
							}
						}
					}
				},
