...
export default {
	methods: {
		...
		getChart4Data() {
			return {
				datasets: [{
					data: [11, 16, 7, 3, 14],
					backgroundColor: ['rgba('+ appVariable.color.themeRgb +', .5)', 'rgba('+ appVariable.color.whiteRgb +', .2)' +', .5)', 'rgba('+ appVariable.color.gray300Rgb +', .5)', 'rgba('+ appVariable.color.gray500Rgb +', .5)', 'rgba('+ appVariable.color.gray800Rgb +', .5)'],
					borderWidth: 0
				}],
		...
		getChart5Data() {
			return {
				...
				datasets: [{
					data: [300, 50, 100],
					backgroundColor: ['rgba('+ appVariable.color.themeRgb +', .5)', 'rgba('+ appVariable.color.whiteRgb +', .2)' +', .5)', 'rgba('+ appVariable.color.themeRgb +', .5)'],
					hoverBackgroundColor: ['rgba('+ appVariable.color.themeRgb +', 1)', 'rgba('+ appVariable.color.whiteRgb +', 1)', 'rgba('+ appVariable.color.gray900Rgb +', 1)'],
					borderWidth: 0
				}]
			};
		},
		getChart6Data() {
			return {
				...
				datasets: [{
					data: [300, 50, 100],
					backgroundColor: ['rgba('+ appVariable.color.themeRgb +', .5)', 'rgba('+ appVariable.color.whiteRgb +', .2)' +', .5)', 'rgba('+ appVariable.color.themeRgb +', .5)'],
					hoverBackgroundColor: [appVariable.color.theme, appVariable.color.white, appVariable.color.gray900],
					borderWidth: 0
				}]
			};
		}
	...
	mounted() {
		...
		axios.get('/assets/data/chart/chartjs-code-7.json').then((response) => {
			this.code7 = response.data;
		});
		axios.get('/assets/data/chart/chartjs-code-8.json').then((response) => {
			this.code8 = response.data;
		});
		axios.get('/assets/data/chart/chartjs-code-9.json').then((response) => {
			this.code9 = response.data;
		});
		axios.get('/assets/data/chart/chartjs-code-10.json').then((response) => {
