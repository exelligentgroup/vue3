	...
	methods: {
		...
		getServerData() {
			return {
				chart: {
					...
					options: {
						colors: ['rgba('+ appVariable.color.whiteRgb + ', .25)', appVariable.color.theme],
						...
						grid: { show: true, borderColor: 'rgba('+ appVariable.color.whiteRgb +', .15)' },
						stroke: { show: false }
					}
				},
				stats: [
		...
		renderMap() {
			...
			var map = new jsVectorMap({
				...
				regionStyle: { initial: { fill: appVariable.color.white, fillOpacity: 0.35, stroke: 'none', strokeWidth: 0.4, strokeOpacity: 1 }, hover: { fillOpacity: 0.5 } },
						...
						<div class="col-lg-6 mb-3 mb-lg-0" v-for="stat in server.stats">
							<div class="d-flex align-items-center">
								...
								<div class="ps-3 flex-1">
									<div class="fs-10px fw-bold text-white text-opacity-50 mb-1">{{ stat.name }}</div>
									...
									<td>
										<div class="d-flex">
											...
											<div class="flex-1 ps-3">
												...
												<div class="fw-500 text-white">{{ product.title }}</div>
												{{ product.price }}
											</div>
										</div>
									</td>
									<td>
										<table class="mb-2">
											<tr>
												...
												<td class="text-white text-opacity-75 fw-500">{{ product.qty }}</td>
											</tr>
											<tr>
												<td class="pe-3">REVENUE:</td>
												<td class="text-white text-opacity-75 fw-500">{{ product.revenue }}</td>
											</tr>
											<tr>
												<td class="pe-3 text-nowrap">PROFIT:</td>
												<td class="text-white text-opacity-75 fw-500">{{ product.profit }}</td>
											</tr>
										</table>
									</td>
						...
						<table class="table table-striped table-borderless mb-2px small text-nowrap">
							<tbody>
								<tr v-if="activityLog" v-for="log in activityLog">
									...
									<td>
										<span class="badge d-block rounded-0 pt-5px w-70px" v-bind:class="{ 'bg-theme text-theme-900': log.highlight, 'bg-white bg-opacity-25': !log.highlight}" style="min-height: 18px">{{ log.badge }}</span>
									</td>
									<td><a href="#" class="text-decoration-none text-white"><i class="bi bi-search"></i></a></td>
								</tr>
								<tr v-else >
